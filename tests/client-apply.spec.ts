...
describe('vision-mix client plugin', () => {
  it('registers separate session-scoped vision and generation tabs when better-sidebar is available', () => {
    ...
    expect(tabs).toEqual([
      expect.objectContaining({ id: 'vision-mix:vision-calls', order: 55, single: true, icon: expect.any(Function), component: expect.any(Function) }),
      expect.objectContaining({ id: 'vision-mix:generation-calls', order: 56, single: true, icon: expect.any(Function), component: expect.any(Function) }),
    ])
    expect((tabs[0]?.title as () => string)()).toBe('识图记录')
    expect((tabs[1]?.title as () => string)()).toBe('生图记录')
    ...
    expect((settingsSections[0]?.label as () => string)()).toBe('Vision Mix')
  })
  ...
  it('keeps the settings section when better-sidebar is absent', () => {
    ...
    expect(settingsSections).toEqual([expect.objectContaining({ id: 'vision-mix' })])
  })
})
