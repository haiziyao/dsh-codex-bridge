...
describe('Bridge GPT settings controller', () => {
  it('returns only model routing and revision metadata', () => {
    const controller = createSettingsController({
      current: () => ROUTING,
      settings: { writable: true, describe: () => [{ ns: 'bridge-gpt', revision: 4 }], mutate: async () => undefined },
    })
  ...
  it('revision-fences one complete routing update', async () => {
    ...
    expect(mutate).toHaveBeenCalledWith('bridge-gpt', [{ op: 'set', path: [], value: routing }], 2)
  })
})
