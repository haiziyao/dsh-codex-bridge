...
import { callsUrl, groupCalls, imageUrl, parseCallsPayload, type VisionCallView } from '../src/client/model.ts'
...
describe('client history model', () => {
  ...
  it('validates the HTTP payload before rendering it', () => {
    ...
    expect(parseCallsPayload({ calls: [{ ...item, sessionId: 'session-a', attachment: {} }] })).toEqual([item])
