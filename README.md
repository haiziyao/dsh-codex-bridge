...
| 操作 | 是否调用 API | 结果 |
|---|---:|---|
| 仅测试图片能力 | 是 | 临时声明 `image`，发送红色测试图，结束后始终恢复原配置 |
| 强制启用 image | 否 | 直接保存 `input: [text, image]` 并设为图片模型，适合已经自行验证过的中转站 |
| 一键测试并配置 | 是 | 正确识别红色才保存并选择该模型；失败自动回滚，推荐使用 |
...
4. 点击“测试生图 API”。测试固定使用 `low`、`1024×1024`、PNG，会产生一次实际生图费用。
5. 看到测试图预览后点击“保存路由”。
